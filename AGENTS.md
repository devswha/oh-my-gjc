...
- `adaptive-response`, `no-english`, `extragoal`, and the `example-plugin` template: no external prerequisites. `workflow-eta` requires Linux, Bun >=1.3.14, its exact-lock private SDK runtime, and a live top-level GJC SDK endpoint.
...
- **The current focused suite has 6 skills and 7 commands.** Skills: `adaptive-response`, `no-english`, SDK-backed `workflow-eta`, `extragoal`, `insane-review`, and read-only `lazycodex-gjc`. Commands: bare `/omg` plus `/omg:setup`, `/omg:gate`, `/omg:gate-always`, `/omg:fable`, `/omg:insane-review`, and `/omg:lazycodex-gjc`.
