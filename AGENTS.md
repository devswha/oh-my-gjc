...
- **The current focused suite has 9 skills and 12 commands.** Skills: `adaptive-response`, `no-english`, SDK-backed `time-left`, `extragoal`, `insane-review`, read-only `lazycodex-gjc`, confirmation-gated `deep-onboarding`, read-only `session-observer`, and explicit-only `orchestrator`. Commands: bare `/omg` plus `/omg:setup`, `/omg:gate`, `/omg:gate-always`, `/omg:no-english`, `/omg:time-left`, `/omg:fable`, `/omg:insane-review`, `/omg:lazycodex-gjc`, `/omg:deep-onboarding`, `/omg:session-observer`, and `/omg:orchestrator`. The three presentation/ETA skills never auto-activate from ordinary natural language; only their explicit commands may load them. `orchestrator` likewise loads only through its explicit command.
...
- **Orchestrator boundary:** `/omg:orchestrator on` applies the threshold delegation policy (2+ files / ~50-line diff / parallel slices → `executor`; verification always retained by the orchestrator) to the current session only and writes nothing. `install [dir]` and `user` are the only writing subcommands: they run `bin/install-orchestrator.sh`, which appends one marker-guarded `## Orchestration policy` section to the target `AGENTS.md` (timestamped backup first, skip when present) and writes exactly two rule files (`.agents/rules/` or `~/.gjc/agent/rules/`). Mechanics are pinned to verified GJC 0.11 behavior: default system prompts do not render `alwaysApply` rules (hence AGENTS.md carries the always-on text), TTSR condition rules fire session-level, AGENTS.md walk-up stops at the repo root, and task-tool sub-agents inherit neither rules nor AGENTS.md so `executor` stays unbound. It never modifies `models.yml`, model config, or credentials, and never lowers verification/approval boundaries.
