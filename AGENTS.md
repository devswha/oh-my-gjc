...
- `lazycodex-gjc`: already installed and logged-in Codex CLI + LazyCodex/OMO; the suite never installs or logs in to them. Codex is NOT required to install the suite: when Codex/systemd/Codex-home are absent, the hardened root `install.sh` skips only the runtime binding (bridge stays fail-closed disabled). After installing Codex, rerun hardened root `install.sh` to repair that binding.
...
- **Native install is REQUIRED, not optional**: gjc 0.9.x auto-exposes plugin commands under the wrong `oh-my-gjc:*` namespace while plugin skills do not surface, so canonical command bodies remain in `templates/` and the hardened one-shot `install.sh` copies 10 skills and 15 commands natively as `/omg:*`. The native installer is manifest-driven and fail-closed, safely merges only the `sol` preset at user scope, and emits one exact mode-`0600` suite-root binding per scope. Asset consumers use its project binding first, then user binding, then a direct repository fallback; they reject a missing or malformed binding rather than selecting any cache. **Bootstrap, upgrade, repair, and post-Codex lazycodex binding repair MUST run root `install.sh`** (one-shot URL or a cloned checkout). Historical newest-cache selection (`sort -V | tail -1`) is non-executable and forbidden because it is not bound to the current plugin install and can execute stale payloads. Open a new session or `/move .` after installation.
