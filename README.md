...
### v0.28.0 cutover and migration

v0.27.0 was the final old-identity bridge. `oh-my-gajae-code` is now the canonical repository, marketplace/plugin identity, source path, and local checkout name.

The canonical installer is `https://raw.githubusercontent.com/devswha/oh-my-gajae-code/main/install.sh`. Old `https://raw.githubusercontent.com/devswha/oh-my-gjc/...` raw URLs do not redirect; old GitHub repository pages and Git remotes redirect, but use the new URL and checkout name in active instructions.

New installs write `oh-my-gajae-code` runtime bindings. The old `oh-my-gjc` binding remains a read-only fallback for at least 30 days or two releases. Old XDG research data, credentials, and `models.yml` are preserved, as is the stable internal `oh-my-gjc:gate-always` marker.
...
- 원문: [`plugins/oh-my-gajae-code/skills/adaptive-response/SKILL.md`](./plugins/oh-my-gajae-code/skills/adaptive-response/SKILL.md)
...
- 원문: [`plugins/oh-my-gajae-code/skills/no-english/SKILL.md`](./plugins/oh-my-gajae-code/skills/no-english/SKILL.md)
...
- 켜기: 스킬 트리거로 활성. 별도 reviewer 프리셋 없이 GJC 기본 모델 구성의 교차세션 리뷰를 쓴다. 원문: [`plugins/oh-my-gajae-code/skills/extragoal/SKILL.md`](./plugins/oh-my-gajae-code/skills/extragoal/SKILL.md)
...
- 원문: [`plugins/oh-my-gajae-code/skills/deep-onboarding/SKILL.md`](./plugins/oh-my-gajae-code/skills/deep-onboarding/SKILL.md)
...
- orchestrator와 no-model finalizer만 프로젝트 밖
  `${XDG_DATA_HOME:-$HOME/.local/share}/oh-my-gajae-code/multi-harness/<repo-id>/<run-id>/`에 쓴다
...
- 원문: [`plugins/oh-my-gajae-code/skills/insane-review/SKILL.md`](./plugins/oh-my-gajae-code/skills/insane-review/SKILL.md)
