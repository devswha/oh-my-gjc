...
- `orchestrator` — top-level 에이전트를 임계값 기반 위임 정책의 오케스트레이터로 전환하고, 프로젝트·사용자 범위 영속 설치 지원(`/omg:orchestrator`)

커맨드 전체: `/omg`, `/omg:setup`, `/omg:gate`, `/omg:gate-always`, `/omg:no-english`, `/omg:time-left`, `/omg:fable`, `/omg:insane-review`, `/omg:lazycodex-gjc`, `/omg:deep-onboarding`, `/omg:session-observer`, `/omg:orchestrator`.
...
### `orchestrator` — 임계값 기반 위임 정책

top-level 에이전트를 오케스트레이터로 전환한다: 컨텍스트 예산은 라우팅·위임 프롬프트 설계·통합·검증에
쓰고, 2+ 파일·~50줄 초과 diff·병렬 슬라이스는 `executor`에 위임한다. "무조건 위임"이 아니다 —
단일 파일 소형 수정·읽기·답변은 직접 하는 게 더 싸고, **모든 최종 검증은 오케스트레이터가 직접**이
비타협 조항이다. task tool 서브에이전트는 rules·AGENTS.md를 상속하지 않으므로 executor는 자유롭게
구현한다(GJC 0.11 검증).

- `/omg:orchestrator on` — 이번 세션에만 적용, 파일 안 씀.
- `/omg:orchestrator install [dir]` — 프로젝트에 영속 설치: `AGENTS.md`에 마커 기반 멱등 append
  (백업 후) + `.agents/rules/` TTSR 트립와이어·`rule://orchestration`. 저장소마다 설치해야 한다
  (AGENTS.md 탐색은 저장소 루트에서 멈춤).
- `/omg:orchestrator user` — `~/.gjc/agent/rules/`에 전역 트립와이어만.
- 원문: [`plugins/oh-my-gjc/skills/orchestrator/SKILL.md`](./plugins/oh-my-gjc/skills/orchestrator/SKILL.md)
