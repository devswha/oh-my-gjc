...
새 세션에서 `/omg`로 목록을 보고 `/omg:setup`으로 설치 상태를 확인합니다. 첫 요청은 다음처럼 구체적으로 적습니다.

| 목적 | 요청 예시 |
|---|---|
| 한국어 우선 표현 켜기 | `/omg:no-english on` |
| ChatGPT 웹에서 코드 검토 | `/omg:insane-review 현재 브랜치의 변경과 관련 파일을 Pro로 검토해` |
| 읽기에 실패한 공개 페이지 조사 | `일반 접근이 403으로 실패한 이 공개 URL의 본문을 조사해: <URL>` |
| 공개 자막 추출 | `이 공개 영상의 한국어 수동 자막을 시간 정보와 함께 추출해: <URL>` |
| 이미지 생성 | `/omg:gpt-image 흰 배경에 파란 가재 아이콘을 만들어` |

일반 한국어 대화는 `no-english`를 켜지 않으며, 일반 코드 검토는 ChatGPT 웹 리뷰를 실행하지 않습니다. 이미지 생성은 `/omg:gpt-image`로 요청합니다.
...
피처 브랜치에 변경을 모두 커밋한 뒤 “이 브랜치를 extragoal로 최종 검토하고, 발견 사항을 수정한 뒤 통과하면 머지해”라고 요청합니다. 전체 게이트는 리뷰뿐 아니라 수정과 머지까지 수행할 수 있습니다. 검토 결과만 필요하면 “머지하거나 수정하지 말고 읽기 전용으로 검토해”라고 범위를 지정합니다.
