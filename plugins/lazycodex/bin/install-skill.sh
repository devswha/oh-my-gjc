#!/usr/bin/env bash
# Install the lazycodex SKILL *and* slash COMMANDs as NATIVE gjc capabilities, so they
...
#
# Native locations:
#   user:    ~/.gjc/agent/skills/<name>/SKILL.md    ~/.gjc/agent/commands/lazycodex:<name>.md
#   project: ./.gjc/skills/<name>/SKILL.md          ./.gjc/commands/lazycodex:<name>.md
#
# Usage:
...
for _a in "$@"; do
  case "$_a" in
    */*|*install-skill.sh)
      ...
      echo "   bash \"\$(ls -d ~/.gjc/plugins/cache/plugins/oh-my-gjc___lazycodex___*/bin/install-skill.sh 2>/dev/null | sort -V | tail -1)\" all" >&2
      exit 2 ;;
  esac
done
