...
EXPECTED_SKILLS=(adaptive-response no-english time-left extragoal insane-review lazycodex-gjc deep-onboarding preset-pack multi-harness-research)
EXPECTED_COMMANDS=(omg setup gate gate-always no-english time-left fable insane-review lazycodex-gjc deep-onboarding preset-pack multi-harness)
EXPECTED_RUNTIMES=(bin/lazycodex-gjc.mjs bin/multi-harness-research.mjs tools/sdk-lab/package.json tools/sdk-lab/bun.lock tools/sdk-lab/src/inspect.ts tools/sdk-lab/src/eta.ts references/preset-pack.yml)
...
# The multi-harness runner has a separate private binding. It deliberately does not
# reuse lazycodex-gjc state: its four adapters have a different executable and exact
# credential-leaf contract. Runtime paths are only accepted when every canonical
...
case "$mode" in
  uninstall)
    ...
    if [ "$target" = "all" ]; then
      ...
      if [ "$scope" = "user" ]; then uninstall_sdk_runtime user; uninstall_multi_harness_runtime; fi
  ...
  user|project)
    ...
    if [ "$mode" = "user" ]; then
      echo "  → adaptive-response, no-english, time-left, and multi-harness-research require their explicit /omg:* commands; other skills keep their documented triggers."
