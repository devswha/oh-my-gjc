...
경로: 이 머신 기준 `~/workspace/oh-my-gjc/plugins/gjc-bugwatch/bin/`. (설치 캐시면 `~/.gjc/plugins/cache/plugins/oh-my-gjc___gjc-bugwatch___*/bin/`로 glob 해석 — `sort -V | tail -1`로 최신. `*gjc-bugwatch*`가 아니라 마켓플레이스 앵커 포함.)
