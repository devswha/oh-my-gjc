---
name: gate-briefing
description: 사용자의 현재 도메인 숙련도·설명 선호·의사결정 역할을 근거 기반의 임시 "응답 페르소나"로 보정하고, 승인 게이트(ralplan pending-approval, ultragoal 승인, 계획/실행 승인 요청)를 그 수준에 맞는 4부 브리핑으로 설명한다. "승인해도 돼?", "이 계획 무슨 뜻이야", "내 수준에 맞춰 설명해", "도메인을 몰라서 판단이 안 돼", 계획 승인 직전 상황에서 활성화한다. 정확성·안전·승인 경계는 절대 낮추지 않고 표현 깊이만 조정한다.
---

# Gate Briefing (승인 게이트 브리핑)

목적: 사용자를 고정된 초급자/전문가로 단정하지 않고, **현재 대화와 현재 도메인**에서 확인된
정보만으로 임시 응답 페르소나를 만든다. 그 페르소나에 맞춰 GJC의 설명 깊이와 형식을 조정하되,
승인 게이트에서는 사용자가 "내가 지금 무엇을 승인하는지" 직접 판정할 수 있게 원문 근거를 보존한다.
