...
description: 멀티벤더 모델 프로파일 프리셋을 ~/.gjc/agent/models.yml에 설치·병합한다. "멀티벤더 프리셋 깔아줘 / grok 프리셋 / sol 프리셋 / codex 프리셋 / mpreset 프로파일 설치 / 역할별 모델 프로파일 세팅" 같은 요청에 활성화. 정본의 grok·sol·codex를 이름 단위로 병합한다.
---
...
요구 로그인: grok = grok-build · openai-codex · anthropic / sol = openai-codex · anthropic / codex = openai-codex 단독
```
