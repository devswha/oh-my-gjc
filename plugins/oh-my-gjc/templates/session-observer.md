---
description: 명시적으로 현재 또는 선택한 GJC 세션을 분리된 tmux 창에서 토큰 없이 읽기 전용으로 관찰한다. 자연어 요청으로는 활성화하지 않는다.
...
인자가 없으면 이 명령을 호출한 현재 tmux 세션을 대상으로 삼는다. 명시 대상은 정확한 `--tmux NAME` 또는
`--session ID` 하나다. 기본값은 `mode=conversation`, `thinking=0`, `follow=1`, `history=20`이며,
thinking 표시는 `--thinking`으로만 켠다. `--mode user-only`는 사용자 메시지만 표시한다. history는
0~200으로 제한한다.

모델은 인자를 다음 안전한 env 필드로 해석해 **한 번의 Bash 호출**에만 전달한다. 사용자 텍스트를
셸 코드에 보간하지 않는다: `OBSERVER_TARGET_KIND`(`current`, `tmux`, `session`; 기본 `current`),
`OBSERVER_TARGET_VALUE`(명시 대상일 때만 설정), `OBSERVER_MODE`, `OBSERVER_THINKING`(`0`/`1`),
