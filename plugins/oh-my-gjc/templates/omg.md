...
- `/omg:orchestrator [on|install [dir]|user|status]` — 오케스트레이터 모드(임계값 기반 위임 정책)를 세션 적용하거나 프로젝트·사용자 범위에 영속 설치.

> `/omg:time-left`, `insane-review`, `lazycodex-gjc`는 필요한 런타임·외부 환경이 없으면 안내하고 안전하게 멈춘다.
> 위의 `/omg:*`가 현재 공개 커맨드 전부다.

## 스킬 (9)
- `adaptive-response`(`/omg:gate*`에서만 명시 호출) · `no-english`(`/omg:no-english`에서만 명시 호출) · `time-left`(`/omg:time-left`에서만 명시 호출) · `extragoal`(외부 최종 리뷰 게이트) · `insane-review` · `lazycodex-gjc`(읽기 전용) · `deep-onboarding`(분석·인터뷰 후 확인된 경로에만 문서 생성) · `session-observer`(`/omg:session-observer`에서만 명시 호출; token-free 읽기 전용) · `orchestrator`(`/omg:orchestrator`에서만 명시 호출; 임계값 기반 위임 정책)
