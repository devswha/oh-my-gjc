...
## 스킬 (6 · 트리거로 자동 활성)
- `adaptive-response`(일반·인터뷰·스킬 응답 보정 + 승인 브리핑) · `no-english`(한국어 우선 표현) · `workflow-eta`(SDK 기반 ralplan/ultragoal 남은 시간) · `extragoal`(외부 최종 리뷰 게이트) · `insane-review` · `lazycodex-gjc`(읽기 전용)
