...
BW="$(resolve_omg_asset "bin/collect.ts")" || exit 1
echo "BW=$BW"
```
...
> **참고 — 자동활성화(스킬)는 hardened `install.sh`로 단일 스위트를 설치할 때 함께 네이티브로 설치된다.**
> 캐시 내부 설치 스크립트를 직접 실행하지 말고, 설치·업그레이드·복구에는 항상 hardened `install.sh`를 다시 실행한다.
