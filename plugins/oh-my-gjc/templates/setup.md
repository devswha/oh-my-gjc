...
for command in easy easy-always plain branchflow-always worktree bugwatch-scan lazycodex-gjc presets release; do
...
- post-v0.17.1 prune는 `easy-answer`, `plain-layer`, `branch-flow`/`worktree`,
  public `gjc-bugwatch`, `lazycodex-gjc`, `multivendor-presets`, `release-gate`의 native
  스킬·커맨드를 제거한다. user scope에서는 LazyCodex binding/receipt도 제거한다.
  기존 `models.yml`과 과거 병합된 프로필은 사용자 소유이므로 수정하지 않는다.
