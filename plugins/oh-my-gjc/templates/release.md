...
3. 리뷰어 기동(작업 맥락 미공유, 읽기 3도구만 — `goal` 등 상태 도구 미노출; 가능하면
   ...
   ```
4. REQUEST_CHANGES ⇒ 블로커 수정 → 커밋 → **새** 리뷰어로 **새 HEAD에** 재서명(사이클당 최대 2라운드).
   verdict 부재/기형/타임아웃도 동일하게 불통과. 한도 소진 시 발행 금지 — 수정을 dev에
   남기고 **사용자에게 에스컬레이션**한다(사용자 지시 시 새 리뷰 사이클로 재개; 게이트는
   릴리스를 영구히 막지 않는다).
