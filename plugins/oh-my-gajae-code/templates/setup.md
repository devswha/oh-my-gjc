...
canonical 진단 대상은 user scope `~/.gjc/agent`다. 아래 6개 skill과 8개 command, 새 canonical suite root binding, private runtime binding의 **존재만** 확인한다. binding 존재는 실제 로그인·selector·credential 검증 성공을 뜻하지 않는다.
