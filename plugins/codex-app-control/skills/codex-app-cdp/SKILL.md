...
> element 매핑(role/name/state, fallback 셀렉터, 완료 신호)은 **잠정(provisional)** 이다.
> 실제 Codex App 버전에 attach한 라이브 스파이크로 확정해야 한다(아래 "스파이크" 참고).
> 하드코딩 CSS 셀렉터에 의존하지 말고 **`tab.observe()` 시맨틱 탐색을 1차 경로**로 쓴다.
...
실제 CDP-enabled Codex App에 attach해 `tab.observe({})` 스냅샷을 떠서
프롬프트 입력 / 전송 버튼 / 생성-중 표시자 / 응답 컨테이너의 role·name·state·value를
표로 확정하고, observe로 부족한 부분만 fallback selector를 추가한 뒤 이 SKILL.md에 반영한다.
스파이크 산출물에 다음을 기록한다: **target URL/title, Codex App 버전, 각 요소의 role/name/state,
fallback selector, 채택한 completion signal, 사용한 stable_ms/timeout_ms, 실패 시 진단 shape.**
