#!/usr/bin/env bash
# Install the codex-cli-control SKILL *and* slash COMMANDs as NATIVE gjc capabilities, so they
...
#
# Native locations:
#   user:    ~/.gjc/agent/skills/<name>/SKILL.md    ~/.gjc/agent/commands/codex-cli-control:<name>.md
#   project: ./.gjc/skills/<name>/SKILL.md          ./.gjc/commands/codex-cli-control:<name>.md
#
# Usage:
...
for _a in "$@"; do
  case "$_a" in
    */*|*install-skill.sh)
      ...
      echo "   bash \"\$(ls -d ~/.gjc/plugins/cache/plugins/oh-my-gjc___codex-cli-control___*/bin/install-skill.sh 2>/dev/null | sort -V | tail -1)\" all" >&2
      exit 2 ;;
  esac
done

PLUGIN_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PLUGIN_NAME="codex-cli-control"
